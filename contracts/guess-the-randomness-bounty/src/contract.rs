...
use cosmwasm_std::{
    to_binary, BankMsg, Binary, Deps, DepsMut, Env, HexBinary, MessageInfo, Response, StdResult,
    WasmMsg,
};
...
use crate::state::{RandomnessData, CONFIG, GUESSES_HISTORY, NOIS_PROXY};
...
pub fn execute(
    ...
    match msg {
        //GuessNextRandomness should be called by a hacker who wants toget the bounty
        ExecuteMsg::GuessNextRandomness { randomness } => {
            execute_guess_next_randomness(deps, env, info, randomness)
        }
...
//execute_guess_next_randomness is the function that will trigger the process of requesting randomness.
//The request from randomness happens by calling the nois-proxy contract
pub fn execute_guess_next_randomness(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    ...
    // Check the format of the randomness is a 32 hex
    let randomness_check: [u8; 32] = randomness
        .to_array()
        .map_err(|_| ContractError::InvalidRandomness)?;

    let job_id = env.block.height.to_string()
        + ":"
        + info.sender.to_string().as_str()
        + ":"
        + randomness.to_string().as_str();
    validate_job_id(&job_id)?;

    GUESSES_HISTORY.save(
        deps.storage,
        (env.block.height, info.sender),
        &RandomnessData {
            guessed_randomness: randomness,
            actual_randomness: None,
    ...
    let response = Response::new().add_message(WasmMsg::Execute {
        ...
        // In this case, the job_id represents the guessed randomness and the address to pay in case of randomness matching
...
pub fn execute_receive(
    ...
    let v: Vec<&str> = job_id.rsplit(":").collect();
    if v.len() != 3 {
        panic!("Strange, how is the job-id not containaing a : separator")
    }

    let block_height = v[0];
    let hacker_address = v[1];
    let guessed_randomness = v[2];

    let guessed_randomness: [u8; 32] = guessed_randomness.to_string()wwj.parse().unwrap();
    if HexBinary::from_hex(guessed_randomness.parse().unwrap()) == randomness {}
    let mut out_msgs = Vec::<BankMsg>::new();

    let amount = deps
        .querier
        .query_balance(env.contract.address.clone(), config.bounty_denom.clone())?;
    let msg = BankMsg::Send {
        to_address: hacker_address.to_string(),
        amount: vec![amount],
    };
    out_msgs.push(msg);
    ...
    .add_attribute("published", published);
    Ok(res)
}
